import { TolarPlugin, NetworkId } from "@tolar/web3-plugin-tolar";
import type { StrHexHash, RpcTxResponse } from "@tolar/web3-plugin-tolar";
...
async function main() {
  try {
    ...
    const retryGetTx = retryDecorator(
      (hash: StrHexHash) => web3.tolar.getTransaction(hash),
