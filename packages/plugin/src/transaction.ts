import type { Bytes, HexString, Transaction } from "web3";
...
import type { RpcTxRequest, NetworkId } from "./types";
...
export class TolTxBody {
  ...
  public toPrintable(): any {
    return {
      senderAddress: this.senderAddress.hexStr,
      receiverAddress: this.receiverAddress.hexStr,
  ...
  public static fromRpcTxRequest(tx: RpcTxRequest): TolTxBody {
    return new TolTxBody(
      new TolAddress(tx.senderAddress),
      new TolAddress(tx.receiverAddress ?? "0x"),
      BigInt(tx.amount),
      BigInt(tx.gas),
      BigInt(tx.gasPrice),
      tx.data,
  ...
  public static fromTransaction(tx: Transaction): TolTxBody {
    ...
    return new TolTxBody(
      ...
      new TolAddress(tx.to ?? ZERO_ADDRESS.hexStr),
      value,
      gas,
      gasPrice,
      data,
  ...
  public toProto(): ProtoTransaction {
    return ProtoTransaction.create({
      senderAddress: this.senderAddress.encodeProto(),
      receiverAddress: this.receiverAddress.encodeProto(),
      value: toU256Proto(this.value),
      gas: toU256Proto(this.gas),
      gasPrice: toU256Proto(this.gasPrice),
      data: utils.hexToBytes(this.data),
      networkId: BigInt(this.networkId),
      nonce: toU256Proto(this.nonce),
    });
  }
}

export function calculateTxBodyHash(protoTxBody: ProtoTransaction): TolHash {
  const rawTxBody = ProtoTransaction.toBinary(protoTxBody);
  return TolHash.fromData(rawTxBody);
}
...
export class TolTx {
  ...
  public toPrintable(): any {
    return {
      bodyHash: this.bodyHash.hexStr,
      signature: this.signature.signature,
      signerId: this.signerId.hexStr,
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
      body: this.body.toPrintable(),
      rawSignedTx: this.rawSignedTx,
      hash: this.hash.hexStr,
    };
  }
  ...
  public static fromBody(txBody: TolTxBody, privateKey: Bytes): TolTx {
    const protoTxBody = txBody.toProto();
    const bodyHash = calculateTxBodyHash(protoTxBody);
