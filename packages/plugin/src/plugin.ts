...
export class TolarPlugin extends Web3PluginBase<TolarExecutionAPI> {
  ...
  public async getNonce(address: StrHexAddress): Promise<TolNum> {
  ...
  public async requestAccounts(): Promise<StrHexAddress[]> {
    return _rpc.requestAccounts(this.requestManager);
  }

  public async getAccounts(): Promise<StrHexAddress[]> {
    return _rpc.getAccounts(this.requestManager);
  }

  public async getCurrentBalance(): Promise<RpcBalanceOutput> {
    return _rpc.getCurrentBalance(this.requestManager);
  }

  public async switchNetwork(network: string | number): Promise<NetworkInfo> {
    return _rpc.switchNetwork(this.requestManager, network);
  }

  public async getPublicKey(): Promise<HexString> {
  ...
  public async sedRawTransaction(tx: RpcTxRequest): Promise<StrHexHash> {
    return _rpc.sendRawTransaction(this.requestManager, tx);
  }
}
